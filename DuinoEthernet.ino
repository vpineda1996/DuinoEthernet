/*
 * Web Server - multi-page.
 * Need an Ethernet Shield over Arduino.
 *
 * By Victor Pineda vpineda1996(at)gmail(dot)com
 *
 * Thanks to Martyn Woerner,Alessandro Calzavara, 
 * alessandro(dot)calzavara(at)gmail(dot)com
...
PROGMEM prog_char content_main_top[] = "<!doctype html><html><head><title>Arduino Web Server</title><style type=\"text/css\">table{border-collapse:collapse;}td{padding:0.25em 0.5em;border:0.5em solid #C8C8C8;}</style></head><body><h1>Arduino Web Server</h1>";
...
// Page 9 for API
PROGMEM prog_char http_uri9[] = "/api";
PROGMEM prog_char content_title9[] = "";
PROGMEM prog_char content_page9[] = "\002";
...
PGM_P http_uris[] PROGMEM = { http_uri1, http_uri2, http_uri3, http_uri4, http_uri5, http_uri9, http_uri6, http_uri7, http_uri8 }; // URIs
  ...
  if (nUriIndex < NUM_PAGES)
  {
    ...
    switch (nUriIndex)
    {
      ...
      case 2:  // page 3
        ...
        break;
      case 5:
        parseJSONAndRespond(client,requestContent);
    }
  }
  else
  {
...
void parseJSONAndRespond(EthernetClient client, BUFFER & requestContent){
    char *pCurrentJSONObject;
    char *pCurrentPlace;
    // Start scanning for the begining of the JSON object
    for(pCurrentJSONObject = strtok(requestContent, "{}"); pCurrentJSONObject != NULL; pCurrentJSONObject = strtok(NULL, "{}")){
        //Serial.println(pCurrentJSONObject); // DEBUG
        // Tells if we are in the name of the variable or the variable itself
        // if it is true we are on the variable place if false then we are in the name of the variable
        bool variablePlace = false;
        int cmd, pin, val, aux, type, addr = -1;
        for (pCurrentPlace = strtok(pCurrentJSONObject, "\"':, "); pCurrentPlace != NULL; pCurrentPlace = strtok(NULL,"\"':, ")){
            if (strcmp(pCurrentPlace, "command") == 0){
                pCurrentPlace = strtok(NULL,"\"':, ");
                cmd = atoi(pCurrentPlace);
                Serial.println(pCurrentPlace);
            }else if (strcmp(pCurrentPlace, "pin") == 0){
                pCurrentPlace = strtok(NULL,"\"':, ");
